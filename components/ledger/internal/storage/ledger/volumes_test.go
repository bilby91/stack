...
func TestGetVolumesWithBalances(t *testing.T) {
	...
	require.NoError(t, store.UpdateAccountMetadata(ctx, "account:1", metadata.Metadata{"category": "1"}))
	require.NoError(t, store.UpdateAccountMetadata(ctx, "account:2", metadata.Metadata{"category": "2"}))
	require.NoError(t, store.UpdateAccountMetadata(ctx, "world", metadata.Metadata{"foo": "bar"}))
	...
	_, err = store.InsertTransaction(ctx, ledger.NewTransaction().
		WithPostings(ledger.NewPosting("world", "account:2", "USD", big.NewInt(50))).
