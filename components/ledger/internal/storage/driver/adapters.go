...
func (d *DefaultStorageDriverAdapter) OpenLedger(ctx context.Context, name string) (ledgercontroller.Store, error) {
	store, err := d.d.OpenLedger(ctx, name)
	if err != nil {
		return nil, err
	}

	return ledgerstore.NewDefaultStoreAdapter(store), nil
}
