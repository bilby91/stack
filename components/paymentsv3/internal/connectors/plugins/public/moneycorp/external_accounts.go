...
import (
	...
	"github.com/formancehq/paymentsv3/internal/connectors/plugins/currency"
...
func (p Plugin) fetchNextRecipients(ctx context.Context, req models.FetchNextExternalAccountsRequest) (models.FetchNextExternalAccountsResponse, error) {
	var oldState recipientsState
	if req.State != nil {
	...
	for page := oldState.LastPage; ; page++ {
		...
		for _, recipient := range pagedRecipients {
			...
			accounts = append(accounts, models.PSPAccount{
				...
				DefaultAsset: utils.Ptr(currency.FormatAsset(supportedCurrenciesWithDecimal, recipient.Attributes.BankAccountCurrency)),
				Raw:          raw,
			})
